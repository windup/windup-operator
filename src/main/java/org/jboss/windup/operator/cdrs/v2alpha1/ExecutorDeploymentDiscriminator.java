...
public class ExecutorDeploymentDiscriminator implements ResourceDiscriminator<Deployment, Windup> {
    @Override
    public Optional<Deployment> distinguish(Class<Deployment> resource, Windup windup, Context<Windup> context) {
        String deploymentName = ExecutorDeployment.getDeploymentName(windup);
